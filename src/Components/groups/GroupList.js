  ...
  render() {
    return (
      ...
      <Table dataSource={this.state.users}>
        ...
        <Column title="Realm Id" dataIndex="realm_id" key="realm_id" />
        ...
        <Column
          ...
          render={(text, record) => (
            <Space size="middle">
              <Button onClick={() => {this.props.history.push(`group/${record.id}/edit`)}}>Edit</Button>
              <Button onClick={() => {this.handleDeleteGroup(record.id)}}>Delete</Button>
            </Space>
          )}
        />
      </Table>
      <Divider />
        <Button type="primary" onClick={() => this.props.history.push(`group/create`)}>Create New</Button>
      </div>
    );
  }
}
