  ...
  render() {
    return (
      ...
      <Table dataSource={this.state.realms}>
        ...
        <Column title="Currency Symbol" dataIndex="currency_symbol" key="currency_symbol" />
        {/* <Column
          title="Tags"
          dataIndex="tags"
          key="tags"
        ...
        <Column
          ...
          render={(text, record) => (
            <Space size="middle">
              <Button onClick={() => {this.props.history.push(`realm/${record.id}/edit`)}}>Edit</Button>
              <Button onClick={() => {this.handleDeleteRealm(record.id)}}>Delete</Button>
            </Space>
          )}
        />
      </Table>
      <Divider />
        <Button type="primary" onClick={() => this.props.history.push(`realm/create`)}>Create New</Button>
      </div>
    );
  }
}
